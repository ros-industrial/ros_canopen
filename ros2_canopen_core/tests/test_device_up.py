...
def generate_test_description():
    ...
    master_node = launch_ros.actions.LifecycleNode(
        ...
        parameters= [os.path.join(path_to_test, "test_device_up.yml")],
    )
...
class TestBasicDevice(unittest.TestCase):
    ...
    def test_sdo(self, launch_service, proc_output):
        ...
        client = self.node.create_client(ros2_canopen_interfaces.srv.COWrite,
        ...
        client = self.node.create_client(ros2_canopen_interfaces.srv.CORead,
            "/basic_device_2/sdo_read")
        
        self.assertTrue(client.wait_for_service(timeout_sec=3.0))

        
        req = ros2_canopen_interfaces.srv.CORead.Request()
        req.index = 0x1017
        req.subindex = 0
        req.type = 16
        future = client.call_async(req)
