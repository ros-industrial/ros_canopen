...
//
//            - Joshua Whitley, Feb 2019.
//    Work based on modifications to https://github.com/alliedmodders/metamod-source
//    by Github user dvander in 2015.
// 11-Feb-19 1.5.1 * Changed compile-time checks to use static_assert.
//                 * Switched to using variadic templates.
...
//
// (a) Error messages are generated by using static_assert.
...
inline OutputClass horrible_cast(const InputClass input)
{
  ...
  static_assert(sizeof(InputClass)==sizeof(u) &&
    sizeof(InputClass)==sizeof(OutputClass), "Cant use horrible cast.");
  u.in = input;
  return u.out;
}
...
class FastDelegate< R (Params...) >
  // Inherit from FastDelegate so that it can be treated just like a FastDelegate
