...
{
public:
    MOCK_METHOD(rclcpp_components::NodeInstanceWrapper, create_node_instance, (const rclcpp::NodeOptions & options), (override));
};

class FakeDeviceContainer
{
public:
    virtual std::vector<rclcpp_components::ComponentManager::ComponentResource> get_component_resources(
        const std::string & package_name, const std::string & resource_index = "rclcpp_components") const
    {
        ...
        components.push_back(rclcpp_components::ComponentManager::ComponentResource("ros2_canopen::CanopenMaster", "canopen_core"));
        components.push_back(rclcpp_components::ComponentManager::ComponentResource("ros2_canopen::LifecycleCanopenMaster", "canopen_core"));
        components.push_back(rclcpp_components::ComponentManager::ComponentResource("ros2_canopen::CanopenDriver", "canopen_core"));
        components.push_back(rclcpp_components::ComponentManager::ComponentResource("ros2_canopen::LifecycleCanopenDriver", "canopen_core"));
...
public:
    ...
    MockDeviceContainer(std::weak_ptr<rclcpp::Executor> executor = std::weak_ptr<rclcpp::executors::MultiThreadedExecutor>())
    : DeviceContainer(executor), fake_device_container()
    {
    }
    MOCK_METHOD(
        bool, load_component,
        (std::string & package_name, std::string & driver_name, uint16_t node_id, std::string & node_name, std::vector<rclcpp::Parameter> & params),
        (override));
    ...
    MOCK_METHOD(std::shared_ptr<rclcpp_components::NodeFactory>, create_component_factory, (const ComponentResource & resource), (override));
    MOCK_METHOD(
        std::vector<ComponentResource>, get_component_resources, (const std::string & package_name, const std::string & resource_index),
        (const, override));
    MOCK_METHOD(void, add_node_to_executor, (rclcpp::node_interfaces::NodeBaseInterface::SharedPtr node_interface), (override));
    ...
    void DelegateToFake()
    {
        ...
        ON_CALL(*this, load_master).WillByDefault([this]() -> bool { return DeviceContainer::load_master(); });
        ON_CALL(*this, load_drivers).WillByDefault([this]() -> bool { return DeviceContainer::load_drivers(); });
        ON_CALL(*this, load_manager).WillByDefault([this]() -> bool { return DeviceContainer::load_manager(); });
        ON_CALL(*this, configure).WillByDefault([this]() { return DeviceContainer::configure(); });
        ON_CALL(*this, get_component_resources).WillByDefault([this](
            const std::string & package_name, const std::string & resource_index) -> std::vector<ComponentResource> {
                return fake_device_container.get_component_resources(package_name, resource_index);
            });
        ON_CALL(*this, add_node_to_executor).WillByDefault([this](rclcpp::node_interfaces::NodeBaseInterface::SharedPtr node_interface) { 
...
{
public:
    explicit MockCanopenDriver(std::string name = "mock_canopen_driver", const rclcpp::NodeOptions & node_options = rclcpp::NodeOptions())
    ...
    //MOCK_METHOD(rclcpp::node_interfaces::NodeBaseInterface::SharedPtr, get_node_base_interface, (), (override));
    rclcpp::node_interfaces::NodeBaseInterface::SharedPtr get_node_base_interface() { return rclcpp::Node::get_node_base_interface(); }
    MOCK_METHOD(void, shutdown, (), (override));
    MOCK_METHOD(bool, is_lifecycle, (), (override));
    MOCK_METHOD(std::shared_ptr<ros2_canopen::node_interfaces::NodeCanopenDriverInterface>, get_node_canopen_driver_interface, (), (override));
};

class MockCanopenMaster : public CanopenMasterInterface, public rclcpp::Node
{
public:
    explicit MockCanopenMaster(std::string name = "mock_canopen_master", const rclcpp::NodeOptions & node_options = rclcpp::NodeOptions())
    : rclcpp::Node(name, node_options)
    {
    }
    MOCK_METHOD(void, init, (), (override));
...
{
    auto services = device_container->get_service_names_and_types_by_node(device_container->get_name(), device_container->get_namespace());
...
TEST_F(DeviceContainerTest, test_load_component_master)
{
    ...
    auto node_instance =
        rclcpp_components::NodeInstanceWrapper(std::static_pointer_cast<void>(node), std::bind(&MockCanopenMaster::get_node_base_interface, node));
    EXPECT_CALL(*device_container, load_component(_,_,_,_,_));
    EXPECT_CALL(*device_container, get_component_resources(_,_));
    EXPECT_CALL(*device_container, create_component_factory(_)).Times(1).WillOnce(Return(node_factory));
...
TEST_F(DeviceContainerTest, test_load_component_driver_non_lifecycle)
{
    ...
    auto node_factory = std::make_shared<MockNodeFactory>();
    auto node_instance =
        rclcpp_components::NodeInstanceWrapper(std::static_pointer_cast<void>(node), std::bind(&MockCanopenDriver::get_node_base_interface, node));
    ...
    EXPECT_THROW(device_container->load_component(package, driver_name, node_id, node_name, params), DeviceContainerException);
...
TEST_F(DeviceContainerTest, test_shutdown)
{
    ...
    auto master_node_instance = rclcpp_components::NodeInstanceWrapper(
        std::static_pointer_cast<void>(master_node), std::bind(&MockCanopenMaster::get_node_base_interface, master_node));
    auto driver_node_instance = rclcpp_components::NodeInstanceWrapper(
        std::static_pointer_cast<void>(driver_node), std::bind(&MockCanopenDriver::get_node_base_interface, driver_node));

    // Load component and expect that size of registered drivers changes to 1
    EXPECT_CALL(*device_container, load_component(_,_,_,_,_));
    EXPECT_CALL(*device_container, get_component_resources(_,_));
    EXPECT_CALL(*device_container, create_component_factory(_)).Times(1).WillOnce(Return(node_factory));
    EXPECT_CALL(*node_factory, create_node_instance(_)).Times(1).WillOnce(Return(master_node_instance));
    device_container->load_component(package, driver_name, node_id, node_name, params);
    ...
    EXPECT_CALL(*device_container, get_component_resources(_,_));
    EXPECT_CALL(*device_container, create_component_factory(_)).Times(1).WillOnce(Return(node_factory));
    EXPECT_CALL(*node_factory, create_node_instance(_)).Times(1).WillOnce(Return(driver_node_instance));
...
TEST_F(DeviceContainerTest, test_load_master_good)
{
    ...
    EXPECT_CALL(*device_container, load_component(_,_,_,_,_)).WillOnce(
        [this, can_master](std::string & package_name, std::string & driver_name, uint16_t node_id, std::string & node_name,
             std::vector<rclcpp::Parameter> & params) -> bool
        {
            device_container->can_master_ = std::static_pointer_cast<ros2_canopen::CanopenMasterInterface>(can_master);
            return true;
        });
...
TEST_F(DeviceContainerTest, test_load_driver_good)
{
    ...
    EXPECT_CALL(*device_container, load_component(_,_,_,_,_)).WillOnce(
        [this, driver](std::string & package_name, std::string & driver_name, uint16_t node_id, std::string & node_name,
             std::vector<rclcpp::Parameter> & params) -> bool
        {
            device_container->registered_drivers_[2] = std::static_pointer_cast<ros2_canopen::CanopenDriverInterface>(driver);
            return true;
        });
    EXPECT_CALL(*device_container, add_node_to_executor(_));
...
TEST_F(DeviceContainerTest, test_init_with_fparam)
{
    ...
    EXPECT_CALL(*device_container, load_drivers()).WillOnce(Return(true));
    EXPECT_CALL(*device_container, load_manager()).WillOnce(Return(true));
    EXPECT_NO_THROW(device_container->init("vcan0", "master.dcf", "bus_configs/good_master_and_two_driver.yml", ""));
}
