...
public:
    ...
    MOCK_METHOD(bool, change_state, (uint8_t node_id, uint8_t transition, std::chrono::seconds time_out));
    ...
    void DelegateToBase()
    {
        ON_CALL(*this, get_state)
            .WillByDefault(
                [this](uint8_t node_id, std::chrono::seconds time_out) -> unsigned int { return LifecycleManager::get_state(node_id, time_out); });
        ...
        ON_CALL(*this, bring_down_master)
            .WillByDefault(
                [this]() -> bool
                { return LifecycleManager::bring_down_master(); });
        ...
        ON_CALL(*this, load_from_config)
            .WillByDefault(
                [this]() -> bool
                { return LifecycleManager::load_from_config(); });
        ...
        ON_CALL(*this, bring_down_all)
            .WillByDefault(
                [this]() -> bool
                { return LifecycleManager::bring_down_all(); });
    }
};
...
TEST_F(LifecycleManagerTest, test_bring_up_master_failed_activate)
{
    EXPECT_CALL(*lifecycle_manager, bring_up_master());
    EXPECT_CALL(*lifecycle_manager, get_state(_, _)).Times(1).WillOnce(Return(1U));
    EXPECT_CALL(*lifecycle_manager, change_state(_, _, _)).Times(2).WillOnce(Return(true)).WillOnce(Return(false));
    EXPECT_FALSE(lifecycle_manager->bring_up_master());
}
...
TEST_F(LifecycleManagerTest, test_bring_down_master)
{
    ...
    EXPECT_CALL(*lifecycle_manager, change_state(_, _, _)).Times(2).WillRepeatedly(Return(true));
    EXPECT_CALL(*lifecycle_manager, get_state(_, _)).Times(1).WillOnce(Return(lifecycle_msgs::msg::State::PRIMARY_STATE_UNCONFIGURED));
...
TEST_F(LifecycleManagerTest, test_bring_down_master_false_state)
{
    EXPECT_CALL(*lifecycle_manager, bring_down_master());
    EXPECT_CALL(*lifecycle_manager, change_state(_, _, _)).Times(2).WillRepeatedly(Return(true));
    EXPECT_CALL(*lifecycle_manager, get_state(_, _)).Times(1).WillOnce(Return(lifecycle_msgs::msg::State::PRIMARY_STATE_ACTIVE));
...
TEST_F(LifecycleManagerTest, test_on_configure)
{
    ...
    EXPECT_EQ(lifecycle_manager->on_configure(state), rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn::SUCCESS);
}
...
TEST_F(LifecycleManagerTest, test_on_configure_fail)
{
    ...
    EXPECT_EQ(lifecycle_manager->on_configure(state), rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn::ERROR);
}
...
TEST_F(LifecycleManagerTest, test_on_activate)
{
    ...
    EXPECT_EQ(lifecycle_manager->on_activate(state), rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn::SUCCESS);
}
...
TEST_F(LifecycleManagerTest, test_on_activate_fail)
{
    ...
    EXPECT_EQ(lifecycle_manager->on_activate(state), rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn::ERROR);
}
...
TEST_F(LifecycleManagerTest, test_on_deactivate)
{
    ...
    EXPECT_EQ(lifecycle_manager->on_deactivate(state), rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn::SUCCESS);
}
...
TEST_F(LifecycleManagerTest, test_on_deactivate_fail)
{
    ...
    EXPECT_EQ(lifecycle_manager->on_deactivate(state), rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn::ERROR);
}

TEST_F(LifecycleManagerTest, test_on_cleanup)
{
    rclcpp_lifecycle::State state;
    EXPECT_EQ(lifecycle_manager->on_cleanup(state), rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn::SUCCESS);
}

TEST_F(LifecycleManagerTest, test_on_shutdown)
{
    rclcpp_lifecycle::State state;
    EXPECT_EQ(lifecycle_manager->on_shutdown(state), rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn::SUCCESS);
}