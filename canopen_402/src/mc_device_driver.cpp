...
namespace canopen_402
{
    ...
    bool DefaultHomingMode::executeHoming()
    {
        ...
        if (obj.get_cached<int8_t, CODataTypes::COData8>() == 0)
        {
            return true;
        }
    ...
    bool Motor402::isModeSupportedByDevice(uint16_t mode)
    {
        ...
        return mode > 0 && mode <= 32 && (supported_drive_modes_.get_cached<uint32_t, CODataTypes::COData32>() & (1 << (mode - 1)));
    }
    ...
    bool Motor402::switchMode(uint16_t mode)
    {
        ...
        if (mode == MotorBase::No_Mode)
        {
            ...
            try
            { // try to set mode
                 op_mode_.set<int8_t, CODataTypes::COData8>(mode);
            }
            catch (...)
            {
            }
        ...
        { // wait for switch
            ...
            else
            {
                while (mode_id_ != mode && std::chrono::steady_clock::now() < abstime)
                {
                    ...
                    op_mode_display_.get<int8_t, CODataTypes::COData8>();                                     // poll
            ...
            else
            {
                ...
                op_mode_.set<int8_t, CODataTypes::COData8>(mode_id_);
            }
        }
    ...
    bool Motor402::readState()
    {
        ...
        uint16_t old_sw, sw = status_word_entry_.get<uint16_t, COData16>(); // TODO: added error handling
        ...
        uint16_t new_mode = monitor_mode_ ? op_mode_display_.get<int8_t, CODataTypes::COData8>() : op_mode_display_.get_cached<int8_t, CODataTypes::COData8>();
    ...
    void Motor402::handleWrite()
    {
        ...
        {
            control_word_entry_.set_cached<uint16_t, CODataTypes::COData16>(control_word_ & ~(1 << Command402::CW_Fault_Reset));
        }
        else
        {
           control_word_entry_.set_cached<uint16_t, CODataTypes::COData16>(control_word_);
        }
    }
