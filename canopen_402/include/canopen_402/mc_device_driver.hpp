...
namespace canopen_402
{
    ...
    {
    public:
        std::shared_ptr<MCDeviceDriver> self;
    ...
    public:
        ...
        void set_cached(const T &other_data)
        {
            data_ = static_cast<uint32_t>(other_data);
        }

        template <typename T, CODataTypes TPY>
        void set(const T &other_data)
        {
            data_ = static_cast<uint32_t>(other_data);
            COData data = {index_, subindex_, data_, TPY};
            auto f = driver_->async_sdo_write(data);
            f.wait();
        }
    ...
    public:
        ...
        ProfiledPositionMode(std::shared_ptr<MCDeviceDriver> driver)
            : ModeTargetHelper(MotorBase::Profiled_Position),
              obj(0x607A, 0U, CODataTypes::COData32, driver)
        {
            driver->objs.push_back(&obj);
        }
        ...
        virtual bool write(OpModeAccesser &cw)
        {
            ...
            if (hasTarget())
            {
                ...
                if ((sw_ & MASK_Acknowledged) == 0 && target != last_target_)
                {
                    ...
                    {
                        obj.set<int32_t, CODataTypes::COData32>(target);
                        cw.set(CW_NewPoint);
                        last_target_ = target;
                    }
                }
    ...
    public:
        DefaultHomingMode(std::shared_ptr<MCDeviceDriver> driver) : obj(0x6098, 0U, CODataTypes::COData32, driver)
        {
            driver->objs.push_back(&obj);
        }
        virtual bool start();
    ...
    public:
        Motor402(const std::string &name, std::shared_ptr<MCDeviceDriver> driver) : MotorBase(name),
                                                                                    ...
                                                                                    status_word_entry_(0x6041, 0U, CODataTypes::COData16, driver),
                                                                                    control_word_entry_(0x6040, 0U, CODataTypes::COData16, driver),
                                                                                    op_mode_display_(0x6061, 0U, CODataTypes::COData8, driver),
                                                                                    op_mode_(0x6060, 0U, CODataTypes::COData8, driver),
                                                                                    supported_drive_modes_(0x6502, 0U, CODataTypes::COData32, driver)
        {
            this->driver = driver;
            driver->objs.push_back(&status_word_entry_);
            driver->objs.push_back(&control_word_entry_);
            driver->objs.push_back(&op_mode_display_);
            driver->objs.push_back(&op_mode_);
            driver->objs.push_back(&supported_drive_modes_);
        }
    ...
    private:
        ...
        RemoteObject status_word_entry_;
        RemoteObject control_word_entry_;
        RemoteObject op_mode_display_;
        RemoteObject op_mode_;
        RemoteObject supported_drive_modes_;
