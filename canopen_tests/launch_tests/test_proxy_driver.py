...
class TestLifecycle(unittest.TestCase):
    ...
    def test_01_to_active(self):
        assert self.node.checkTransition("lifecycle_device_manager_node", State.PRIMARY_STATE_UNCONFIGURED, Transition.TRANSITION_CONFIGURE), "Could not configure device manager"
        assert self.node.checkTransition("lifecycle_device_manager_node", State.PRIMARY_STATE_INACTIVE, Transition.TRANSITION_ACTIVATE), "Could not configure device manager"
    ...
    def test_07_to_unconfigured(self):
        assert self.node.checkTransition("lifecycle_device_manager_node", State.PRIMARY_STATE_ACTIVE, Transition.TRANSITION_DEACTIVATE), "Could not configure device manager"
        assert self.node.checkTransition("lifecycle_device_manager_node", State.PRIMARY_STATE_INACTIVE, Transition.TRANSITION_CLEANUP), "Could not configure device manager"
